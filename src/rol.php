...
$usuarios = mysqli_query($conexion, "SELECT * FROM empleados WHERE ID_EMPLEADO = $id");
$consulta = mysqli_query($conexion, "SELECT * FROM detalle_permisos WHERE ID_EMPLEADO = $id");
...
if (isset($_POST['permisos'])) {
    ...
    mysqli_query($conexion, "DELETE FROM detalle_permisos WHERE ID_EMPLEADO = $id_user");
    if ($permisos != "") {
        foreach ($permisos as $permiso) {
            $sql = mysqli_query($conexion, "INSERT INTO detalle_permisos(ID_EMPLEADO, ID_PERMISO) VALUES ($id_user,$permiso)");
            if ($sql == 1) {
